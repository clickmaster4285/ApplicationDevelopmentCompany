...
import platformsData from "@/content/platforms";
...
export async function generateStaticParams() {
  return platformsData.pages.map((page: any) => ({
    slug: page.url
      .replace(/^\/+|\/+$/g, "") // Remove leading/trailing slashes
      .replace("platforms/", ""), // Remove the platforms/ prefix
  }));
}
...
export async function generateMetadata({
  ...
  const pageData = platformsData.pages.find((page: any) => {
  ...
  if (!pageData) {
    return {
      ...
      description: "The requested platform page could not be found.",
    };
  }
...
export default async function PlatformsPage({
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-white/20">
      <HeroHeader pageData={pageData} type="Platform" />
